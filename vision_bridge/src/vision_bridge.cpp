...
int getVisionType(const std::string & encoding)
{
...
{
  int source_type = getVisionType(source.encoding);
...
// Internal, used by toVisionCopy and cvtColor
VisionImagePtr toVisionCopyImpl(
  const cv::Mat & source,
...
{
  // Copy metadata
  VisionImagePtr ptr = std::make_shared<VisionImage>();
  ...
  if (dst_encoding.empty() || dst_encoding == src_encoding) {
    ...
    for (size_t i = 0; i < conversion_codes.size(); ++i) {
      ...
      if (conversion_code == SAME_FORMAT) {
        ...
        int image2_type = CV_MAKETYPE(VISION_MAT_DEPTH(getVisionType(dst_encoding)), image1.channels());
...
sensor_msgs::msg::Image::SharedPtr VisionImage::toImageMsg() const
{
...
void VisionImage::toImageMsg(sensor_msgs::msg::Image & ros_image) const
{
  ...
  if (image.isContinuous()) {
    ...
    uchar * vision_data_ptr = image.data;
    for (int i = 0; i < image.rows; ++i) {
      memcpy(ros_data_ptr, vision_data_ptr, ros_image.step);
      ros_data_ptr += ros_image.step;
      vision_data_ptr += image.step;
    }
  }
}

// Deep copy data, returnee is mutable
VisionImagePtr toVisionCopy(
  const sensor_msgs::msg::Image::ConstSharedPtr & source,
  const std::string & encoding)
{
  return toVisionCopy(*source, encoding);
}
  ...
  const std::string & encoding)
{
  ...
  return toVisionCopyImpl(matFromImage(source), source.header, source.encoding, encoding);
}

// Share const data, returnee is immutable
VisionImageConstPtr toVisionShare(
  const sensor_msgs::msg::Image::ConstSharedPtr & source,
  const std::string & encoding)
{
  return toVisionShare(*source, source, encoding);
}
...
VisionImagePtr cvtColor(
  const VisionImageConstPtr & source,
  const std::string & encoding)
{
  return toVisionCopyImpl(source->image, source->header, source->encoding, encoding);
}

/////////////////////////////////////// CompressedImage ///////////////////////////////////////////

sensor_msgs::msg::CompressedImage::SharedPtr VisionImage::toCompressedImageMsg(const Format dst_format)
const
{
...
void VisionImage::toCompressedImageMsg(
  ...
  const Format dst_format) const
{
  ...
  if (encoding == enc::BGR8 || encoding == enc::BGRA8  || encoding == enc::MONO8 || encoding == enc::MONO16)
  {
    ...
    VisionImagePtr tempThis = std::make_shared<VisionImage>(*this);
    VisionImagePtr temp;
...
VisionImagePtr toVisionCopy(const sensor_msgs::msg::CompressedImage & source, const std::string & encoding)
{
  // Construct matrix pointing to source data
  ...
  if (encoding != enc::MONO16) {
    ...
    {
      case 4:
        return toVisionCopyImpl(rgb_a, source.header, enc::BGRA8, encoding);
        break;
      case 3:
        return toVisionCopyImpl(rgb_a, source.header, enc::BGR8, encoding);
        break;
      case 1:
        return toVisionCopyImpl(rgb_a, source.header, enc::MONO8, encoding);
        break;
      default:
        return VisionImagePtr();
    }
  }
  else {
    return toVisionCopyImpl(rgb_a, source.header, enc::MONO16, encoding);
  }
}

VisionImageConstPtr cvtColorForDisplay(
  const VisionImageConstPtr & source,
  ...
  const CvtColorForDisplayOptions options)
{
  ...
  if (!source) {
    throw Exception("vision_bridge.cvtColorForDisplay() called with empty image.");
  }
  ...
  if (encoding.empty()) {
    ...
    // We could have vision_bridge exception or std_runtime_error
    // from sensor_msgs::image_codings routines
    catch (const std::runtime_error & e) {
      throw Exception(
              "vision_bridge.cvtColorForDisplay() output encoding is empty and cannot be guessed.");
    }
  } else {
    ...
    {
      throw Exception(
              "vision_bridge.cvtColorForDisplay() does not have an output encoding \
  ...
  {
    VisionImagePtr result(new VisionImage());
  ...
  if (min_image_value != max_image_value) {
    if (enc::numChannels(source->encoding) != 1) {
      throw Exception(
              "vision_bridge.cvtColorForDisplay() scaling for images \
               with more than one channel is unsupported");
    }
    VisionImagePtr img_scaled(new VisionImage());
  ...
  VisionImagePtr source_typed(new VisionImage());
  ...
  } catch (vision_bridge::Exception & e) {
    throw Exception(
            "vision_bridge.cvtColorForDisplay() while trying to convert image from '" +
